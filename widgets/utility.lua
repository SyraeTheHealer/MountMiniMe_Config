...
local MountMiniMe = _G.MountMiniMe

function Addon:CreateClass(...)
	return MountMiniMe:CreateClass(...)
end
...
function Addon:CreateNameGenerator(prefix)
	...
	return function()
		id = id + 1
		return ('%s_%s_%d'):format('MountMiniMeOptions', prefix, id)
	end
end

-- a thing to manage rendering stuff on the next frame
-- will probably push into MountMiniMe since its generally useful
do
	local subscribers = {}
